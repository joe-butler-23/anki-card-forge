    ...
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4">
      <div className="text-center space-y-2">
        ...
        <p className="text-slate-500 dark:text-slate-400">
          You have forged <span className="font-bold text-indigo-600 dark:text-indigo-400">{validCards.length}</span> cards.
        </p>
      </div>
        ...
        <div className="max-h-[50vh] overflow-y-auto">
          {validCards.map((card, index) => (
            <div key={card.id} className="p-4 border-b border-slate-100 dark:border-slate-800 last:border-0 flex items-start gap-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
              <span className="text-xs font-bold text-slate-300 dark:text-slate-600 mt-1">#{index + 1}</span>
      ...
      {!isConnected ? (
        <div className="text-center text-sm text-slate-500 dark:text-slate-400">
          ...
          <button onClick={onConnect} className="text-indigo-600 dark:text-indigo-400 underline font-semibold">
            Connect
          </button>{' '}
          to sync or download JSON.
        </div>
      ) : null}
    </div>
  );
